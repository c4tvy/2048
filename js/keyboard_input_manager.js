...
KeyboardInputManager.prototype.listen = function () {
  ...
  this.bindButtonPress(".undo-button", this.undo); // 新增撤销
  ...
  function isInteractiveElement(target) {
    return target.closest('.restart-button, .keep-playing-button, .retry-button, .theme-toggle-button, .undo-button');
  }
  ...
  document.addEventListener('mouseup', function (event) {
    ...
    var dx = mouseEndX - mouseStartX;
    var dy = mouseEndY - mouseStartY;
    var absDx = Math.abs(dx);
    var absDy = Math.abs(dy);

    var threshold = Math.max(30, Math.round(gameContainer.clientWidth * 0.08));
    var axisBias = 6;
